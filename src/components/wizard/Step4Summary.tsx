...
export function Step4Summary() {
  ...
  const generateNaturalLanguageSummary = () => {
    ...
    const rewardDescriptions = rewards.map((reward) => {
      if (reward.type === "percentDiscount" || reward.type === "PERCENTAGE_DISCOUNT") {
                  ...
                  <span className="text-text-gray">
                    {rule.type === "category" || rule.type === "TARGET_CATEGORY"
                      ...
                      : rule.type === "customerTag"
                        ...
                        : (rule.type === "minAmount" || rule.type === "MIN_AMOUNT")
                          ? `$${rule.value}`
                          : rule.value}
                  </span>
                </div>
          ...
          <div className="flex flex-wrap gap-3">
            {formData.rewards?.map((reward) => (
              ...
              >
                ...
                {reward.type === "percentDiscount" || reward.type === "PERCENTAGE_DISCOUNT"
                  ? `${reward.value}%`
                  : reward.type === "fixedDiscount" || reward.type === "FIXED_DISCOUNT"
                    ? `$${reward.value}`
